...
int exe1()
{
	...
	int a,b,op,answer,user_answer,i;
	int used[101][101][2]={0};//三位数组记录 
	float rate;
	time_t start,end;
	double duration;
	...
	for(i=0;i<total;i++)
	{
		...
		while(used[a][b][op]==1||a==0);//题目重复，不合要求的重新生成 
		used[a][b][op]=1;
		if(op==0)
		{
			answer=a*b;
	...
	if(rate>=90)
	{
		printf("优秀！！！你对乘除法的掌握程度已经达到熟练的程度了，继续保持呀！\n"); 
	}
	else if(rate>=70&&rate<90)
	{
		printf("良好！！你已经大部分掌握乘除法了，勤加练习，相信你离满分不远了！\n"); 
	}
	else if(rate>=60&&rate<70)
	{
		printf("及格！多多练习，注意掌握方法，注意细节，相信下一个乘除法大神就是你！\n"); 
	}
	else if(rate>=0&&rate<60)
	{
		printf("啊欧，这次没有及格呀，建议先了解乘除法本质后，从简单练起，再来挑战哦，等着你下一次凯旋！\n");
	}
	return 0; 
}
void exe2()
{
	printf("\033[38;5;202m欢迎使用整数乘除法练习器！\033[0m\n");
	printf("\n");
	printf("本练习器会随机生成100以内的两个正整数和乘除运算符\n");
